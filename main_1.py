...
class tk_master:
    ...
    def select_camera(self):
        ...
        ##
        admin_id2 = canvas1.create_text(390, 100, text="SELECT CAMERA", font=("Times New Roman", 24),fill=get_data.get_text_color())

        e1 = Entry(user_home_root, font=('times', 15, ' bold '), width=20)
        canvas1.create_window(390, 170, window=e1)


        def upload_files(event):
            tk_master.camera=str(e1.get())
            print(tk_master.camera)

            if tk_master.camera=="":
                messagebox.showinfo("Alert","Enter Place Name",parant=user_home_root)
            else:
        ...
        admin_id1 = canvas1.create_text(390, 250, text="Start", font=("Times New Roman", 28), fill=get_data.get_text_color())
